...
This folder packages everything needed to pull the fine‑tuned QLoRA adapter from Cloud Storage, install `vLLM`, and expose the AML model behind an OpenAI‑compatible API on an Ubuntu VM (GPU or CPU fallback). The workflow assumes the adapters were uploaded by `python -m training.llm_training.common.train --method qlora --upload_output_to gs://...` or by the Vertex pipeline (`training/cloud-training-script/submit_vertex_job.sh`).
...
1. Identify the artifact path (each training run uploads to `gs://finery-training/models/<job-name>`).
   ...
   ```
2. Pull it down with the helper script (defaults to `serve/serve-llm/artifacts`).
   ```bash
   ./serve/serve-llm/download_artifacts.sh "$MODEL_GCS_URI"
   # or: ARTIFACTS_DIR=/models/aml ./serve/serve-llm/download_artifacts.sh "$MODEL_GCS_URI"
   ```
...
export ADAPTER_DIR="serve/serve-llm/artifacts"           # wherever download_artifacts.sh synced
export PORT=8000
export HOST=0.0.0.0
./serve/serve-llm/run_vllm_server.sh
```
...
## 7. Operational notes
- **Autosave artifacts** under `/opt/aml-llm/serve/serve-llm/artifacts` to reuse across VM rebuilds. The `.gitignore` keeps these blobs out of Git.
...
- **Upgrades** – To update weights, rerun `download_artifacts.sh` with the new `gs://` path and restart the server. To update dependencies, rerun `pip install -r serve/serve-llm/requirements.txt --upgrade`.
