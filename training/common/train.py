...
def create_model(
    *,
    ...
    """Load the base model and optionally wrap it with LoRA/QLoRA adapters."""
    model_kwargs = {}
    ...
    if method == "qlora":
        try:
            ...
            raise ImportError(
                "bitsandbytes is required for QLoRA. Install it via "
...
def main():
    ...
    parser.add_argument(
        "--packing",
        action="store_true",
        help="Enable dataset packing (requires flash attention kernels).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--bf16",
        action="store_true",
        help="Enable bf16 mixed precision where supported.",
    )
    parser.add_argument(
        "--upload_output_to",
        type=str,
        default=None,
        help="Optional gs:// URI to upload the trained artifacts.",
    )
    parser.add_argument(
        "--hf_token",
        type=str,
        default=None,
        help="Optional Hugging Face Hub token.",
    )
    parser.add_argument(
        "--lora_rank",
        type=int,
        default=32,
        help="LoRA rank (QLoRA and LoRA modes only).",
    )
    parser.add_argument(
        "--lora_alpha",
        type=int,
        default=16,
        help="LoRA alpha (QLoRA and LoRA modes only).",
    )
    parser.add_argument(
        "--lora_dropout",
        type=float,
        default=0.05,
        help="LoRA dropout (QLoRA and LoRA modes only).",
    )
    ...
    split_dataset = full_dataset.train_test_split(
        test_size=args.val_split_size, shuffle=True, seed=42
    )

    ds_train = split_dataset["train"].map(
        to_chat, remove_columns=["facts", "decision", "fired_rules"]
    )
    ds_val = split_dataset["test"].map(
        to_chat, remove_columns=["facts", "decision", "fired_rules"]
    )
    ...
    tok = AutoTokenizer.from_pretrained(
        args.model_id, token=hf_token, use_fast=True
    )
